...
document.addEventListener("DOMContentLoaded", () => {
    ...
    stars.forEach(star => {
        star.addEventListener("click", () => {
            selectedRating = Number(star.getAttribute("data-value")); // selected rating
    ...
    // Load saved reviews from localStorage (unique key for Devera)
    const savedReviews = JSON.parse(localStorage.getItem("deveraReviews")) || [];
...
function addReview() {
    ...
    localStorage.setItem("deveraReviews", JSON.stringify(savedReviews));
...
function updateAverageRating() {
    ...
    // Convert ratings to numbers and filter valid
